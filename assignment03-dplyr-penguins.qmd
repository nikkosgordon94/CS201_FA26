...
Hints: You can always c heck your work using `unique()`. It'll tell you the unique values that exist in a specified column, and will also help you identify the correct spelling

```{r, messages=FALSE}
unique(penguins$species) # an example of how to use unique()

#filter female penguins within 2 islands and culmen length
data1 <- penguins %>%
  filter(sex == "FEMALE" &
         culmen_length_mm >= 39 &
...
```{r, message=FALSE, warning=FALSE}
#exclude comment variable using select function
data2 <- penguins |>
  select(study_name, 
         sample_number, 
         species,
...
#could do select(-comment) as well
```
...
```{r, message=FALSE, warning=FALSE}
#relocation of select variables and column postioning

data3 <- penguins |> 
  relocate(individual_id, .before = species)
#can use .before or .after in relocate
```
...
```{r, message=FALSE, warning=FALSE}
#renaming current column names to new column names
data4 <- penguins |> 
  rename(island_name = island) #to make multiple us , to seperate
#view specified variable
view(data4)
```
...
data5 <- penguins |>
  mutate(flipper_length_km = flipper_length_mm / 1e6) |> 
  rename(flippometer = flipper_length_km)
```
...
number8 <- penguins |> 
  ...
  summarise(culmen_depth_mean = mean(culmen_depth_mm, na.rm = TRUE), culmen_length_mean = mean(culmen_length_mm, na.rm = TRUE))
#na.rm is to remove any NA factors
```
...
#    TRUE ~ sex
#  ))
  #previous code changes all sex Observations to NA
count <- penguins|> 
...
summary(as.factor(casesize$size))
#if else is fork, case when defines each category
```
...
library(janitor)
library(plotly)
interactive <- ggplot(data = final_boss, aes(x = flipper_rank, y = flipper_length_mm, color = species, shape = island)) +
  geom_point() +
  theme(legend.position = "none") 

ggplotly(interactive)
#compare date over hover reveales more detailed information
```
