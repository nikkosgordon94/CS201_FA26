...
#reads csv from laptop - csv is dataframe
```
...
#can import data frame from URL 
#Scraping can be turned off to prevent URL link
```
...
penguins_data <- penguins_data |> 
  clean_names()
#Allows all names to follow same style
```
...
#Added 10 to get 1st 10
tail(penguins_data, 5)

install.packages("skimr") #Make sure package is installed
...
- Type your answer(s) in bullet points below
  ...
  - dplyr, dibble, dlookr, forcast, pastecs, Hmisc, psych, DataExplore

- Let's check one out DataExplore
...
#Working directory will ensure you're in the right project - getwd() in Console. Can list files to ensure everything matches - list.files() 
#setwd("data") - can assign working directory (wd)
#setwd ("..") clears where wd is set
#list.files("") - by inputing folder name will create a list of files contained
```
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  geom_histogram()
#instead of fill can use color, color will outline
#data show less green penguins
```
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
  ...
  theme_bw() + #Gets rid of gray, black and white
  labs(title = "Flipper Length of Palmers Archipelago penguins",
       subtitle = "Three species observed in Palmer Archipelago",
       x = "Flipper Length (mm)",
       y = "Number of Penguins")
  
#sclae fill manual is to adjust the colors of current fill - manually insert
#alpha = 0.05, 0.25, 0.5, 0.75 adds transparency to visualization
```

Manually choosing colors is cool, but there are also color palettes available for us with `viridis`

```{r, message=FALSE}
#color scales is good to lock at - inferno, magma, etc
ggplot(data = penguins_data, aes(x = flipper_length_mm, fill = species)) +
...
#Color palettes for _a:
#Color palettes for _b:
#Color palettes for_c:
#Color palettes for _d:
```
...
ggplot(data = penguins_data, aes(y = flipper_length_mm, fill = species)) +
  geom_boxplot()
#Changed flipper length from x value to y value
...
ggplot(data = penguins_data, aes(x = species, y = flipper_length_mm, fill = species)) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1), legend.position = "none") +
         coord_flip()
#Had to add x-axis to use geom point, use geom jitter instead to get rid of close clusters
#Added scale fill and labs from above
```
...
ggplot(data = penguins_data, aes(x = flipper_length_mm, y = body_mass_g, color = species, shape = island)) +
  geom_point() +
  scale_shape_manual(values = c(2,4,15)) #changing scale of shape manually
...
=======
?shape
>>>>>>> 0a664de953c7dbc1839ad13364cf0fa664874f17
...
#color is used for shapes 
#scale shape values range from 1 to 24, 
```

Cool! Let's add species to this plot. What about islands
...
library(ggplot2)
library(plotly)
p <- ggplot(data = penguins_data, aes(x = flipper_length_mm, y = body_mass_g, color = species)) +
  geom_point(alpha = .65) +
  theme_bw() +
  scale_fill_viridis_d(option = "plasma") +
  labs(title = "Flipper Length of Palmers Archipelago penguins",
       subtitle = "Three species observed in Palmer Archipelago",
       x = "Flipper Length of Penguins by Species",
       y = "Weight of Penguins in Gram")
ggplotly(p)
```
